...
class AppState {
    constructor() {
        ...
        this.conversationMode = {
            continuous: true,  // 是否为持续对话模式
            active: true,      // 当前对话是否活跃
            historyCount: 0    // 对话历史数量
        };

        // 用户会话管理
        // 页面刷新时清除之前保存的用户ID，确保开始新会话
        const oldUserId = localStorage.getItem('saved_user_id');
        if (oldUserId) {
            console.log('🧹 页面刷新检测到旧的用户ID，正在清除:', oldUserId);
            localStorage.removeItem('saved_user_id');
        }
        localStorage.removeItem('session_timestamp'); // 同时清除时间戳
    ...
    clearSavedUserId() {
        ...
        if (oldUserId) {
            console.log('🗑️ 清除保存的用户ID:', oldUserId);
            localStorage.removeItem('saved_user_id');
...
let audioConfig = null; // 将在appState初始化后设置
    ...
    async attemptReconnect(endpoint, onMessage) {
        ...
        try {
            ...
            if (window.appState && window.appState.savedUserId) {
                ...
                console.log('自动重连包含保存的用户ID:', window.appState.savedUserId);
            }
...
/**
 * 只启动录音，不重新创建WebSocket连接（用于继续对话场景）
 */
async function startRecordingOnly() {
    ...
    // 检查WebSocket连接是否存在
    if (!websocket || websocket.readyState !== WebSocket.OPEN) {
        throw new Error('WebSocket连接不存在或未打开');
    }
...
const MessageHandler = {
    ...
    messageHandlers: {
        ...
        'tts_audio': function(data) {
            // 音频参数已在 tts_start 下发，这里只需传入音频帧本体
            // 检查编码方式，兼容新旧格式
            const encoding = data.encoding || 'latin-1';  // 默认为旧格式
        ...
        'tts_error': function(data) {
            ...
            // 恢复状态，让用户知道可以继续对话
            DOMUtils.updateTexts({
                status: currentLanguage === 'zh' ? '⚠️ 语音合成失败，但可以继续对话' : '⚠️ TTS failed, but you can continue the conversation'
            });
        ...
        'conversation_paused': function(data) {
            console.log('对话已暂停:', data.message);
        ...
        'conversation_restarted': function(data) {
            ...
            // 如果不是持续对话模式，自动开始录音（不重复创建WebSocket连接）
            if (window.appState && !window.appState.conversationMode.continuous && !isStreaming) {
                ...
                if (window.recordingStartTimer) {
                    ...
                    console.log('⚠️ 清除之前的录音启动定时器');
                }

                window.recordingStartTimer = setTimeout(async () => {
                    console.log('🎤 开始自动录音流程，当前WebSocket状态:', websocket?.readyState);
                    try {
                        await startRecordingOnly(); // 只启动录音，不重新创建WebSocket
                        console.log('✅ 一次性对话模式：自动开始录音成功');
                    } catch (error) {
                        console.error('❌ 一次性对话模式：自动开始录音失败:', error);
                    } finally {
                        window.recordingStartTimer = null;
                    }
                }, 800); // 延迟800ms确保WebSocket连接和UI更新完成
            }
        },

        // 新增：处理连接即将关闭的通知
        'connection_closing': function(data) {
            console.log('收到连接关闭通知:', data.message, '原因:', data.reason);
            ...
            // 如果是一次性对话完成，不需要特殊处理，等待自然断开
            if (data.reason === 'one_time_conversation_complete') {
                console.log('一次性对话完成，等待连接自然断开');
            }
        }
    },
...
/**
 * 继续对话（用于一次性对话模式，保留历史记录）
 */
async function restartConversation() {
    console.log('开始继续对话流程');
...
function showConversationPaused(data) {
    ...
    $restartButton.show().focus(); // 自动聚焦到按钮
    ...
    showConfigChangeNotification() {
        const $notification = $(`
            <div style="position: fixed; top: 20px; right: 20px; background: #ffc107; color: #856404;
                ...
                ${currentLanguage === 'zh' ? '需要重新开始对话以应用新设置' : 'Need to restart conversation to apply new settings'}
            </div>
        `);
...
const LANGUAGE_DATA = {
    zh: {
        ...
        // 对话模式相关
        continuousMode: '持续对话模式',
        oneTimeMode: '一次性对话模式',
        conversationPaused: '💤 对话已暂停',
        conversationRestarted: '对话已重启',
        continueConversation: '🔗 继续对话',
        restartConversation: '重新开始对话',
        conversationCompleted: '一次性对话完成',
        historyPreserved: '历史记录已保留，点击上方按钮继续对话',
        reconnecting: '正在重新连接...',
        continuingConversation: '正在继续对话...',
...
$(document).ready(function() {
    // 初始化应用状态（只创建一次，确保localStorage清理生效）
